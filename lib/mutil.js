		...
		find: function(arg, root) {
			...
			if(arg.match(/,/)) {
				return arg.split(',').inject([], function(output) {
					return output.concat(_this.find(this.trim()).except(output));
				});
			}
			...
			if(arg.match(/ /)) {
				var parts = arg.split(' '),
				    parent = parts.shift(),
				    selector = parts.join(' ');
				return _this.find(selector, _this.find(parent, root));
			}
			...
			if(arg.match(/[#a-zA-Z0-9\-_]+\.[a-zA-Z0-9\-_.]+/)) {
				var parts = arg.split('.'),
				    tagname = parts.shift();
				return _this.find(tagname, root).select(function() {
					var el = this;
			...
			if(typeof arg == 'string') return this.nodelistToArray(root.getElementsByTagName(arg));
