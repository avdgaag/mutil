		...
		var find = function(arg, root) {
			...
			if(typeof root.length != 'undefined') {
				return root.inject([], function(output) {
					return output.concat(find(arg, this).except(output));
				});
			}
			...
			if(arg.match(/,/)) {
				return arg.split(',').inject([], function(output) {
					return output.concat(find(this.trim()).except(output));
				});
			}
			...
			if(arg.match(/ /)) {
				var parts = arg.split(' '),
				    parent = parts.shift(),
				    selector = parts.join(' ');
				return find(selector, find(parent, root));
			}
			...
			if(arg.match(/[#a-zA-Z0-9\-_]+\.[a-zA-Z0-9\-_.]+/)) {
				var parts = arg.split('.'),
				    tagname = parts.shift();
				return find(tagname, root).select(function() {
					var el = this;
