...
Enumerable.prototype = {
	 ...
	 * </pre>
	 *
	 * Since the callback function works outside of the native javascript
	 * loop, there is no way to use `break` or `continue`. Simulating continue+
	 * is easy enough using `return`, jumping to the next element in the
	 * collection, but `break` is a little trickier. This is solved like
	 * Prototype does it: throwing a special exception value, which
	 * is caught internally, triggering a `break`:
	 *
	 * <pre class="code">
	 * var first_value_greater_than_two = null;
	 * [1, 2, 3, 4].each(function() {
	 *   first_value_greater_than_two = i;
	 *   if(this > 2) {
	 *     console.log(this);
	 *     throw('break');
	 *   }
	 ...
	 * </pre>
	 *
	 * Not using `throw('break')` would have resulting in `4` rather than `3`.
	 *
	...
	/**
	 * Select only those elements from the array that are not in the given
	 * array of elements to exclude.
	 *
	 * You can use a single argument, multiple arguments or an array of
	 * elements to exclude from this array.
	 *
	 * Usage:
	 *
	 * <pre class="code">
	 ...
	 * </pre>
	 *
	 * @param {Array} elements_to_exclude to remove from this array
	 * @returns a new array with all elements from this array without those from elements
	 * @type Array
	 */
	except: function(elements_to_exclude) {
		// If there are multiple arguments, use those as the elements to exclude
		if(arguments.length > 1) elements_to_exclude = Array.prototype.slice.call(arguments);

		if(typeof elements_to_exclude.length == 'undefined') {
			return this.select(function() {
				return this != elements_to_exclude;
			});
		} else {
			return this.select(function(i, arr) {
				var found = false;
				for(var i = 0, j = elements_to_exclude.length; i < j; i++) {
					if(elements_to_exclude[i] == this) found = true;
				}
				return !found;
			});
		}
	...
	/**
	 * See if all elements in an array return true for the given callback.
	 *
	 * Usage:
	 *
	 * <pre class="code">
	 * [1,2,3].all(function() { this > 0 });
	 * [1,2,3].all(function() { this > 2 });
	 * </pre>
	 *
	 * Produces:
	 *
	 ...
	 * </pre>
	 *
	 * @param {Function} callback is the test to run on every element
	 * @returns whether all elements comply
	 * @type Bool
	 * @since v0.1.1
	 */
	all: function(callback) {
		if(this.length == 0) return false;
		return this.inject(true, function(memo, i, arr) {
			return memo && callback.call(this, i, arr);
		});
	},
	...
	collect: function(callback) {
		return this.inject([], function(memo, i, arr) {
			memo.push(callback.call(this, i, arr));
			return memo;
		});
	}
};
...
/**
 * Provide a fallback for the console on modern browsers to
 * older browsers (ie IE). Simply alert any message passed
 * to console.log().
 *
 * @todo untested
 */
...
(function() {
	function Mutil() {
		if(typeof arguments[0] == 'function' && (!Function.prototype.call || typeof(arguments[0].call) == 'function')) {
			Mutil.addLoadEvent(arguments[0]);
		} else {
			this.elements = Mutil.prototype.find(arguments[0], arguments[1]);
		}
	}

	/**
	 * Add a load event to the chain of functions to fire when the document
	 * fires onLoad event is triggered.
	 *
	 * Usage:
	 *
	 * <pre class="code">
	 * $(function() {
	 *   alert('I show up when the page has loaded!');
	 * });
	 * </pre>
	 *
	 ...
	 * @since 0.1.2
	 */
	Mutil.addLoadEvent = function(func) {
	...
	Mutil.prototype = {
		// List of available selectors. Add your own if you want to.
		selectors: [
			...
			function(arg, root) {
				if(matches = arg.match(/^#([a-zA-Z0-9\-_]+)$/)) {
					var el = root.getElementById(matches[1]);
					return el ? [el] : [];
				}
			},
			...
			function(arg, root) {
				...
				if(arg.match(/,/)) {
					return arg.split(',').inject([], function(output) {
						return output.concat(_t.find(this.trim()).except(output));
					});
				}
			},
			...
			function(arg, root) {
				if(arg.match(/ /)) {
					var parts = arg.split(' '),
					    parent = parts.shift(),
					    selector = parts.join(' ');
					return this.find(selector, this.find(parent, root));
				}
			},
			...
			function(arg, root) {
				if(arg.match(/[#a-zA-Z0-9\-_]+\.[a-zA-Z0-9\-_]+$/)) {
					var parts = arg.split('.'),
					    tagname = parts.shift();
					return this.find(tagname, root).select(function() {
						var el = this;
						return parts.all(function() {
							return el.className.match(this, 'gi');
						});
					});
				}
			},
			...
			function(arg, root) {
				if(typeof arg == 'string' && arg.match(/^\w+$/)) return this.nodelistToArray(root.getElementsByTagName(arg));
			}
		],

		find: function(arg, root) {
			// Reference to the current finder function
			var _this = this;

			// Helper method to convert a nodelist to an array
			this.nodelistToArray = function(nodelist) {
				var arr = [];
				for(var i = 0, j = nodelist.length; i < j; i++) { if(nodelist[i].nodeType == 1) arr.push(nodelist[i]); }
				return arr;
			};
			...
			if(typeof root.length != 'undefined') {
				return root.inject([], function(output) {
					return output.concat(_this.find(arg, this).except(output));
				});
			}

			// Loop over all available selectors and return the first match
			var output = null;
			Mutil.prototype.selectors.each(function(func) {
				var result = this.call(_this, arg, root);
				if(undefined != result) {
					output = result;
					throw 'break';
				}
			...
			// Return either the resulting elements or raise an exception
			if(output) {
				return output;
			} else {
				throw('Invalid selector ' + arg);
			}
		},
		...
		collect:function() { return Enumerable.prototype.collect.apply(this.elements, arguments); },
		any:    function() { return Enumerable.prototype.any    .apply(this.elements, arguments); },
		all:    function() { return Enumerable.prototype.all    .apply(this.elements, arguments); },
		except: function() { return Enumerable.prototype.except .apply(this.elements, arguments); },
	...
	window.mutil = window.$ = function() { return new Mutil(arguments[0], arguments[1]); };
