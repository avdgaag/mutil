Mutil.Query.selectors = {
    by_tag_name: new Mutil.Selector(/^\w+$/, function(selector, root) {
        return root.getElementsByTagName(selector);
    }),

    by_id: new Mutil.Selector(/^#[a-z][a-z0-9\-_]*$/i, function(selector, root) {
        return [root.getElementById(selector.replace('#', ''))];
    }),

    multiple: new Mutil.Selector(/[^,]+(,[^,]+)+[^,]*/, function(selector) {
        var selectors  = selector.split(/,\s*/),
            output     = [];

        for(var i = 0, j = selectors.length; i < j; i++) {
            var result = new Mutil.Query(selectors[i]);
    ...
    by_class_name: new Mutil.Selector(/^(\w+)?(\.[a-z][a-z0-9\-_]*)+$/, function(selector) {
        ...
        if(element !== '') {
            var output = [];
            var results = new Mutil.Query(element);
    ...
    adjacent: new Mutil.Selector(/[^+]+(\+[^+]+)+[^+]*/, function(selector) {
        var selectors = selector.split(/\s*\+\s*/),
            output    = new Mutil.Query(selectors.shift());
        for(var i = 0, j = selectors.length; i < j; i++) {
            var temp_collection = [];
            for(var k = 0, l = output.length; k < l; k++) {
                ...
                if(ns && Mutil.Query.matches(ns, selectors[i])) {
    ...
    child: new Mutil.Selector(function(selector) {
        return (/[^+>,~]\s+(?![+>~])/).test(selector.replace(/\[.+\]/, ''));
    }, function(selector) {
        var selectors = selector.split(/\s+(?!\+>~)/),
            output    = new Mutil.Query(selectors.shift());
        for(var i = 0, j = selectors.length; i < j; i++) {
            output = new Mutil.Query(selectors[i], output);
        }
        return output;
    }),

    direct_descendant: new Mutil.Selector(/>/, function(selector) {
        var parts  = selector.split(/\s*>\s*/),
            output = new Mutil.Query(parts.shift());
        for(var i = 0, j = parts.length; i < j; i++) {
            ...
            for(k = 0, l = output.length; k < l; k++) {
                ...
                for(m = 0, n = children.length; m < n; m++) {
                    if(Mutil.Query.matches(children[m], parts[i])) {
